...
export class TitleScene extends Phaser.Scene {
    ...
    create(): void {
        var background: Phaser.GameObjects.Image = this.add.image(
            0,
            this.cameras.main.height -
                this.textures.get('background').get(0).height,
            'background'
        );
        background.setScrollFactor(0);
        background.setOrigin(0);
        background.setDepth(-1000);
        var startBtn: BaseButton = new BaseButton(
            this,
        ...
        this.add.text(
            this.cameras.main.centerX - 60 - 40,
            this.cameras.main.height - 32,
            String.fromCharCode(169) + ' 2019-2020 James Cote',
            {
                color: '#fff',
            }
        );
        this.add.text(this.cameras.main.width - 72, 16, `v${GAME_VERSION}`, {
            color: '#fff',
        });
